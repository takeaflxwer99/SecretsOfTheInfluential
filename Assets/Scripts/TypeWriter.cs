...
public class TypeWriter : MonoBehaviour
{
    ...
    public IEnumerator TypeWriterString(string content, TextMeshProUGUI textMesh, float typerSpaceTime = 0.05f)
    {
        IsTyping = true;
        IsFastTyper = false;
        ...
        for (int i = 0; i < contentCharArray.Length; i++)
        {
            ...
            else
            {
                ...
                if (typeStack.Count > 0)
                {
                    string[] typeStr = typeStringBuilder.ToString().Split(new char[] { '>' }, System.StringSplitOptions.RemoveEmptyEntries);
